...
- (void)addDownloadToQueue:(NSDictionary *)downloadInfo
{
    __block KBYTDownloadOperation *downloadOp = [[KBYTDownloadOperation alloc] initWithInfo:downloadInfo completed:^(NSString *downloadedFile) {
        ...
        NSLog(@"download completed: %@", downloadOp);
...
{
    NSDictionary *info = @{@"status": @"",@"percentComplete": [NSNumber numberWithFloat:-1] };
   [[NSNotificationCenter defaultCenter] postNotificationName:@"updateProgressNote" object:nil userInfo:info];
    [[[self operations] firstObject] start];
    DLog(@"[self operations] count: %lu %@", [self operations].count, self.operations);
...
+ (id)sharedInstance {
    ...
    if (!shared){
        dispatch_once(&onceToken, ^{
            ...
            shared.operationQueue.maxConcurrentOperationCount = 1;
        });
    }
